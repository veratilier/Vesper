...
import { queueStickerCollection } from "@/lib/stickers";
...
export async function POST(request: Request) { if (!(await authorizeApp(request))) return json(request, { error: "Device not paired" }, 401); try { return json(request, await queueStickerCollection(await memoryScopeFromRequest(request), await request.json() as Record<string, unknown>)); } catch (error) { return json(request, { error: error instanceof Error ? error.message : "Could not add to the collection queue" }, 400); } }
