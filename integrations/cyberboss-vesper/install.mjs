...
if (!targetRoot || targetRoot === process.cwd()) throw new Error('Usage: node install.mjs /absolute/path/to/cyberboss');
...
const appUpdated = appOriginal
  .replace('const { createWeixinChannelAdapter } = require("../adapters/channel/weixin");', 'const { createWeixinChannelAdapter } = require("../adapters/channel/weixin");\nconst { createVesperChannelAdapter } = require("../adapters/channel/vesper");')
  .replace('function createRuntimeAdapter(config) {', 'function createChannelAdapter(config) {\n  return config.channel === "vesper" ? createVesperChannelAdapter(config) : createWeixinChannelAdapter(config);\n}\n\nfunction createRuntimeAdapter(config) {')
  .replace('this.channelAdapter = createWeixinChannelAdapter(config);', 'this.channelAdapter = createChannelAdapter(config);');
const configUpdated = configOriginal.replace('channel: readTextEnv("CYBERBOSS_CHANNEL") || "weixin",', 'channel: readTextEnv("CYBERBOSS_CHANNEL") || "weixin",\n    vesperBaseUrl: readTextEnv("CYBERBOSS_VESPER_BASE_URL") || "https://api.vesper.r-vera.com",\n    vesperBridgeToken: readTextEnv("CYBERBOSS_VESPER_TOKEN"),\n    vesperUserId: readTextEnv("CYBERBOSS_VESPER_USER_ID") || "vesper-user",\n    vesperPollIntervalMs: readIntEnv("CYBERBOSS_VESPER_POLL_INTERVAL_MS") || 1500,');
if (appUpdated === appOriginal || configUpdated === configOriginal) throw new Error('CyberBoss source shape was not recognized; no files were changed.');
await mkdir(adapterDir, { recursive: true });
await copyFile(path.join(sourceDir, 'index.js'), path.join(adapterDir, 'index.js'));
