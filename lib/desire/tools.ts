...
export const desireTools = [
  { name: 'desire_status', description: 'Read the original private Desire state and real-interaction timestamps. Reading never scores an encounter.', schema: z.object({}) },
  { name: 'desire_history', description: 'Read original Desire history, newest first, with stable pagination.', schema: z.object({ limit: z.number().int().min(1).max(50).optional(), cursor: z.string().max(256).optional() }) },
